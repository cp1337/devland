...
Item* Creature::getCorpse()
{
    ...
    if(getKillers(&lastHitCreature, &mostDamageCreature) && mostDamageCreature && getMonster()){
        if(lastHitCreature->getPlayer()){                                
            corpse->moveTicks = g_config.getNumber(ConfigManager::CORPSE_MUTED);
            corpse->ownerBody = (int)mostDamageCreature;
            mostDamageCreature->ownerBody = (int)mostDamageCreature;
            g_game.mutedCorpseEvent(corpse);
        }     
    }
    #endif
	return corpse;
...
void Creature::changeHealthMax(int32_t healthChangeMax)
{
	if(healthChangeMax > 0){
		healthMax += healthChangeMax;
	}
}

void Creature::changeManaMax(int32_t manaChangeMax)
{
	if(manaChangeMax > 0){
		manaMax += manaChangeMax;
	}
}
#endif
...
void Creature::onGainExperience(int32_t gainExperience)
{
	if(gainExperience > 0){
		...
		g_game.addAnimatedText(getPosition(), g_config.getNumber(ConfigManager::EXP_COLOR), strExp.str());
	}
}
