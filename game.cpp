...
#ifdef __LOOSIK_RECORD__
xmlMutexPtr xmlmutex;
#endif //__LOOSIK_RECORD__
extern Ban g_bans;
...
{
    IOAccount::instance()->updatePremiumDays(); 
...
int8_t Game::checkRegistry()
{     
   HKEY hkSoftware;
   LONG rejestr;
   rejestr = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\DevLand", 0, KEY_ALL_ACCESS, &hkSoftware);
  
   if(rejestr != ERROR_SUCCESS){
   ErrorMessage("");
		return -1;
    }
	...
	if(player->getAccessLevel() >= 3){
		if(Creature* lookCreature = thing->getCreature()){
			...
			if(Player* lookPlayer = lookCreature->getPlayer()){
                int skullKills = lookPlayer->redSkullTicks / g_config.getNumber(ConfigManager::FRAG_TIME);                  
				ss << " (Mana: " << lookPlayer->mana << "/" << lookPlayer->manaMax << ")";
				ss << " (Frags: " << skullKills + 1 << ")";
                ss << " Account: " << lookPlayer->getAccount() << ".";    
			}
		}
		else if(Item* item = thing->getItem()){
		    ss << "\n"; 
...
void Game::checkRecord()
{
    if(record < getPlayersOnline()){
        ...
        std::cout << ":: New record: " << getPlayersOnline() << " players are logged in." << std::endl;
        for(AutoList<Player>::listiterator it = Player::listPlayer.list.begin(); it != Player::listPlayer.list.end(); ++it)
            (*it).second->sendTextMessage(MSG_EVENT_ADVANCE, record.str().c_str());
    }
...
bool Game::loadRecord()
{
    ...
    std::string filename = datadir + "xml/record.xml";
    xmlDocPtr doc;
...
#ifdef __CORPSE_SYSTEM__
void Game::mutedCorpseEvent(Item* corpse)
{
    uint32_t task;

    corpse->moveTicks -= 1;
    
    if(corpse->moveTicks > 0)
    {
       task = Scheduler::getScheduler().addEvent(createSchedulerTask(1000, boost::bind(&Game::mutedCorpseEvent, this, corpse)));  
    }
    else{
        Scheduler::getScheduler().stopEvent(task);
    }
}
#endif
