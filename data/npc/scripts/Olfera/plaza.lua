...
function onCreatureSay(cid, type, msg)
  	...
  	elseif focus == cid then
		talk_start = os.clock()
	     ...
	     elseif msgcontains(msg, 'sail') or msgcontains(msg, 'sailing') or msgcontains(msg, 'beach') then
	          selfSay('So, You want go to Orelfera beach?')
		  sail = 1		  

	     elseif msgcontains(msg, 'yes') then
				if sail == 1 and pay(cid,30) then
					travel(cid, 432, 525, 7)
					selfSay('Set the sails!')
					focus = 0
					talk_start = 0

				elseif sail == 1 and not pay(cid,30) then
