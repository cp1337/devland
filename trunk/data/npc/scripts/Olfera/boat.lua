...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
			selfSay('Hello ' .. creatureGetName(cid) .. '! I can take you to Karmia, Anshara, Inferna, Venore or Folda.(70gp everywhere)')
  	...
  	elseif focus == cid then
		talk_start = os.clock()
	     ...
	     elseif msgcontains(msg, 'sail') or msgcontains(msg, 'sailing') then
	          selfSay('Where do you want to go? To Orelfera, Telmun?')
	     

	     elseif msgcontains(msg, 'orelfera') then
		  selfSay('Do you seek a passage to Orelfera for 120 gold?')
		  sail = 1

	     elseif msgcontains(msg, 'telmun') then
		  selfSay('Sorry, but on way to Telmun is Storm right now... So try ask me later')
		  sail = 2		  


	     elseif msgcontains(msg, 'yes') then
				if sail == 1 and pay(cid,120) then
					travel(cid, 262, 633, 6)
