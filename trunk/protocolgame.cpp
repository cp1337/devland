...
void ProtocolGame::sendCreatureSkull2(const Creature* creature)
{
	if(canSee(creature)){
		NetworkMessage* msg = getOutputBuffer();
		if(msg){
...
{
	#define MAX_NUMBER_OF_OUTFITS 35
	...
	if(msg){
	           ...
	           for(OutfitListType::const_iterator it = player_outfits.begin(); it != player_outfits.end() && (counter < MAX_NUMBER_OF_OUTFITS); ++it, ++counter){
