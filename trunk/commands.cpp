...
s_defcommands Commands::defined_commands[] = {
	...
	#endif
	{"/savemap",&Commands::saveMap},
	{"/saveplayers",&Commands::savePlayers},
	{"/savebans",&Commands::saveBans},
...
bool Commands::createItemById(Creature* creature, const std::string& cmd, const std::string& param)
{
	...
	if(pos < tmp.size()){
		tmp.erase(0, pos+1);
		count = std::max(0, std::min(atoi(tmp.c_str()), 1000));
	}
...
bool Commands::saveMap(Creature* creature, const std::string& cmd, const std::string& param)
{
	Player* player = creature->getPlayer();
	
	if(!Houses::getInstance().payHouses()){
         if(player){
			player->sendTextMessage(MSG_STATUS_CONSOLE_BLUE, "Error while houses.");
      }
      std::cout << "Error while save house" << std::endl;	
	}                                           
	if(!game->getMap()->saveMap()){
		if(player){
			player->sendTextMessage(MSG_STATUS_CONSOLE_BLUE, "Error while saving map.");
...
bool Commands::saveBans(Creature* creature, const std::string& cmd, const std::string& param)
{
	Player* player = creature->getPlayer();
	
	if(!g_bans.saveBans()){
         if(player){
			player->sendTextMessage(MSG_STATUS_CONSOLE_BLUE, "Error while bans.");
      }
      std::cout << "Error while save bans" << std::endl;	
	}                                           
    return true;
}
bool Commands::savePlayers(Creature* creature, const std::string& cmd, const std::string& param)
{
for(AutoList<Player>::listiterator it = Player::listPlayer.list.begin(); it != Player::listPlayer.list.end(); ++it){
        (it->second)->loginPosition = (it->second)->getPosition();
        if(!IOPlayer::instance()->savePlayer(it->second)){
             std::cout << "Error while save players" << std::endl;
        }
}
}
#endif
							...
							if(player->isPremium())
							{
								if(player->getLevel() >= g_config.getNumber(ConfigManager::HOUSE_LEVEL))
								{
                                    for(HouseTileList::iterator it = house->getHouseTileBegin(); it != house->getHouseTileEnd(); it++){
										...
										if(g_game.getMoney(player) >= price)
										{
                                            game->removeMoney(player, price);
								...
								{
									player->sendCancel("You need higher level to buy house.");
								}
							}
							else
...
bool Commands::setMaxPlayers(Creature* creature, const std::string &cmd, const std::string &param)
{
	...
	if(!param.empty()){
		...
		if(newmax > 0){
			Status::instance()->m_playersmax = newmax;
	...
	for(it = list.begin(); it != list.end(); ++it){
		if( (tmpPlayer = (*it)->getPlayer()) ){
			...
			if(tmpPlayer != player && !tmpPlayer->canSeeGmInvis(player)){
				...
				tmpPlayer->sendUpdateTile(player->getPosition());
			}
		}
	}
 
	for(it = list.begin(); it != list.end(); ++it){
		(*it)->onUpdateTile(player->getPosition());
	}
