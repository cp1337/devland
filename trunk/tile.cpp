...
void Tile::onRemoveTileItem(uint32_t index, Item* item)
{
	...
	for(it = list.begin(); it != list.end(); ++it){
		if((tmpPlayer = (*it)->getPlayer())){
			...
			tmpPlayer->sendRemoveTileItem(cylinderMapPos, i, item);
    #else
	...
	for(it = list.begin(); it != list.end(); ++it) {
		if((tmpPlayer = (*it)->getPlayer())){
			...
			if(!creature->isGmInvis() || tmpPlayer->canSeeGmInvis(creature)){
				...
				tmpPlayer->sendCreatureMove(creature, toPos, fromPos, oldStackPos-i, teleport);
			}
            #else
	...
	if(const Creature* creature = thing->getCreature()){
		...
		if(const Monster* monster = creature->getMonster()){
			...
			if(hasFlag(TILESTATE_BLOCKSOLID) || (hasBitSet(FLAG_PATHFINDING, flags) && hasFlag(TILESTATE_NOFIELDBLOCKPATH))){
				if(!(monster->isGhost() || hasBitSet(FLAG_IGNOREBLOCKITEM, flags) ) ){
					return RET_NOTPOSSIBLE;
				}
			}
