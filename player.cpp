...
void Player::addSkillAdvance(skills_t skill, uint32_t count)
{
	...
	if(skills[skill][SKILL_TRIES] >= vocation->getReqSkillTries(skill, skills[skill][SKILL_LEVEL] + 1)){
		...
		g_creatureEvents->playerAdvance(this, (AdvanceType_t(skill+3)), skills[skill][SKILL_LEVEL], skills[skill][SKILL_LEVEL]+1);
        #endif
...
void Player::addManaSpent(uint32_t amount)
{
	if(amount != 0 && !hasFlag(PlayerFlag_NotGainMana)){
		...
		if(manaSpent >= reqMana){
            #ifdef __ON_ADVANCE__
		    g_creatureEvents->playerAdvance(this, ADVANCE_MAGLVL, magLevel, magLevel+1);
            #endif                 
			manaSpent -= reqMana;
			magLevel++;
...
void Player::addExperience(uint32_t exp)
{
	...
	if(prevLevel != newLevel){
        #ifdef __ON_ADVANCE__
		g_creatureEvents->playerAdvance(this, ADVANCE_LVL, prevLevel, newLevel);
        #endif            
		level = newLevel;
		updateBaseSpeed();
...
void Player::onGainExperience(int32_t gainExperience)
{
	...
	if(party && party->isSharedExperienceActive() && party->isSharedExperienceEnabled()){
		party->shareExperience(gainExperience);
...
void Player::changeHealthMax(int32_t healthChangeMax)
{
	Creature::changeHealthMax(healthChangeMax);
	sendStats();
}

void Player::changeManaMax(int32_t manaChangeMax)
{
	Creature::changeManaMax(manaChangeMax);
	sendStats();
}
#endif
...
void Player::addUnjustifiedDead(const Player* attacked)
{
	...
	if(redSkullTicks >= g_config.getNumber(ConfigManager::BAN_UNJUST) * g_config.getNumber(ConfigManager::FRAG_TIME)){
		g_bans.addPlayerBan(getGUID(), std::time(NULL) + g_config.getNumber(ConfigManager::BAN_TIME));
		kickPlayer();
	}
}
...
void Player::genReservedStorageRange()
{
	...
	for(it = outfits.begin(); it != outfits.end(); ++it){
		...
		uint32_t premium = (*it)->premium;
		if(!global_outfits.isInList(looktype)){
			int32_t value = (looktype << 16) | (premium & 0xFF);
...
uint32_t Player::getOutfitAddon(uint32_t lookType)
{
	KnowAddonMap::iterator it = knowAddonMap.find(lookType);
    if(it != knowAddonMap.end()){
		return it->second;
