...
bool IOPlayer::loadPlayer(Player* player, const std::string& name, bool preload /*= false*/)
{
	...
	query.str("");
	query << "SELECT * FROM player_addons WHERE player_id='" << player->getGUID() << "'";
	if((result = db->storeQuery(query.str()))){
		do{
            uint32_t lookType, addon = 0;
			lookType  = result->getDataInt("outfit");
			addon = result->getDataInt("addon");            
	...
	if((result = db->storeQuery(query.str()))){
        do{
            ...
            death.time   = result->getDataInt("date");            
