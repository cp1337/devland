	...
	if(attacker){
		if(const Player* targetPlayer = target->getPlayer()){
			...
			if(const Player* attackerPlayer = attacker->getPlayer()){
				...
				uint32_t protectionLevel = g_config.getNumber(ConfigManager::PROTECT_LEVEL);
	            if(target->getPlayer()->getLevel() < protectionLevel || attacker->getPlayer()->getLevel() < protectionLevel)
		           return RET_YOUMAYNOTATTACKTHISPLAYER;
			}
